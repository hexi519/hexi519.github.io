...
​	广泛的数据包级仿真结果表明：1）与几种广泛使用的基准方法相比，DRL-TE显着<u>降低了端到端延迟</u>，并不断提高了网络实用性，<u>同时提供了更好或相当的吞吐量</u>；  2）DRL-TE对网络的变化更具有鲁棒性； 和3）DRL-TE始终优于最新的DRL方法（用于连续控制），即深度确定性策略梯度（DDPG），但是它不能提供令人满意的性能。
...
是我看到的几篇文章中比较formal的，不知道是不是因为跟投的是INFOCOMM有关，会喜欢写公式。但是这个思路和叙述是总体看下来最顺畅的一个，先提出我们的问题是什么，把模型跑出来再去谈算法。也就是说，这里是建模的时候抛开了RL，解这个规划问题的时候才说是用RL去解决，比CC那边直接上来就定义这是个RL问题让人好多了，当然，也可能是因为CC那边不好出一个规划模型的数学表达就是了，因为实际上在单条链路上跑，动作对象也很少，不好写成规划模型的形式。

下次~~不知道是讲MRTE比较合适呢，~~还是讲Pensive吧。~~前者不知道感觉大家感觉还需不需要再详细讲一下，~~后者就是一个是HongziMao，建模会比较有经验，另一个是好多文章都提到了这个。还是得去好好研究一下。

下一步规划，就是去看看多商品流问题和~~一些传统的解决方法把~~（后者不讲，是因为要凑之前工作量的数量），很多文章里面提到了，但是我之前其实大概知道是个资源分配问题。
